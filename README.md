...
兼容 DeepSeek thinking 模式返回的 `reasoning_content`。非流式响应会保存该字段，流式响应会逐片重组；只要 assistant 消息仍在历史中，后续请求就会原样回传其 `reasoning_content`，包括工具调用、完成门禁和后续用户轮次。思维内容不会作为可见回答流式展示。
