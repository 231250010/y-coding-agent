...
- `task_list.py`：有界任务状态、`update_task_list` 工具和稳定 system 锚点。
...
兼容 DeepSeek thinking 模式返回的 `reasoning_content`。非流式响应会保存该字段，流式响应会逐片重组，并区分“字段缺失”和“服务端明确返回 null”；只要 assistant 消息仍在可回放协议历史中，后续请求就会原样回传该字段，包括工具调用、完成门禁和后续用户轮次。若 thinking 会话的最终草稿缺失该字段且完成门禁仍需续轮，草稿不会先写入下一次模型请求；即使该回答已经作为最终可见文本返回，也不会污染后续协议历史。恢复旧会话时会清理这类不可回放草稿及其孤立工具结果。原始思维内容不会发送到浏览器。模型在工具调用响应的可见 `content` 中给出简短的目标、依据和下一步动作时，Agent 会将其脱敏、限制为最多 500 个字符并保存成可折叠的“决策摘要”；没有可见内容时不额外生成摘要，也不会增加一次模型调用。
...
- 实时回答：模型文本通过流式 Chat Completions 增量显示；正在生成的消息只更新自身内容，不重建历史消息和侧栏，避免闪烁与滚动抖动。本机 SSE 通道主动推送状态，断线时由低频 `/api/state` 轮询恢复。
- 工作目录：可调用本机目录选择器，也可输入绝对路径；不同对话可以绑定不同目录。
- 文件改动：每轮任务结束后只显示一次“本轮改动”，不会为每次工具调用重复生成入口。
- 完成证据：修改后必须有属于当前修改版本的成功测试、构建或静态检查；否则 Agent 会再获得一次补充验证机会，并将最终状态明确标为“未验证”。
- 右侧工作台：点击改动文件进入 Diff；桌面和平板宽度下预览作为自适应第三列参与布局，不覆盖中间对话区，手机窄屏切换为全屏预览。点击页头“发布台”可查看 Compose 连接、发布门禁、各环境锁状态、服务健康、活动版本和版本时间线。
...
- 任务飞行计划：多阶段任务会显示目标、完成比例、当前阶段和阻塞原因；清单由模型通过结构化工具维护，并独立于聊天摘要持久化。
...
流式文本只保存在运行时内存中，并以有界频率通知网页，不会按 token 写入会话文件。用户消息、上下文压缩结果、每条完整 `ToolResult` 和最终答复形成检查点并原子持久化；因此进程异常退出后不会只留下磁盘改动而丢失对应工具证据。流式工具调用会按索引重组名称与 JSON 参数，完整后再交给本地工具层执行。
...
| `run_process` | 以参数数组执行测试、构建或静态检查 | `shell=False`；参数、风险、审批、超时和输出均受控，并可形成完成证据 |
...
5. 旧轮次的超长工具输出在送入摘要器前按“头部 + 尾部”压缩；最近两轮默认保持完整，只有仍超过硬性上下文上限时才从较早的工具结果开始自适应截断，并保留输出尾部的错误与总结信息。

多阶段任务另有一份独立于聊天历史的结构化任务清单。状态限制为 `pending`、`in_progress`、`completed` 和 `blocked`，最多 20 项、最多一个 `in_progress`，且阻塞项必须提供原因。任务清单写入本机会话存储；每次模型调用前，程序把当前快照投影为位于第一条用户消息之前的唯一 system 状态消息，因此轮次摘要不会裁剪它。该消息明确标记为数据而非指令，字段和长度均经过校验，避免把自由文本变成无限增长的高权限提示。

单个任务在以下任一情况终止：模型给出通过完成证据门禁的最终文本、达到最大步骤数、连续三次出现相同工具错误、用户停止，或模型 API 返回不可恢复错误。文件修改会递增独立的修改版本；只有当前版本之后成功执行的已识别测试、构建、静态检查或 Compose 验证才覆盖该版本。验证失败或没有验证时，结果状态为 `completed_unverified`，网页显示“已完成 · 未验证”，不会把普通文字总结冒充验证证据。限流、连接失败和超时采用有限次数指数退避；认证和请求参数错误不重试。
...
自动化测试使用脚本化假模型、假 Docker Runner、本机临时 MCP stdio Server 和回环 HTTP Server，不访问真实 API、Docker Engine 或远程服务器。覆盖 Agent 循环、完成证据门禁、工具级检查点、参数数组验证命令、Skill 指令/资源渐进披露与受控脚本、MCP tools/resources/prompts、动态工具通知、roots/ping 反向请求、sampling 审批/预算/隔离/递归保护、stdio 重连、HTTP POST/GET SSE/Session、环境隔离、审批、脱敏、工具协议、文件与命令安全、Git 与 Compose 参数数组、DevOps 环境边界、部署验证、上下文压缩、会话持久化、本机 HTTP 边界、网页状态机、项目/对话管理和 Diff 展示。
