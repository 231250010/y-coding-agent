...
3. `CodingAgent` 在预算阈值处把旧滚动摘要和新淘汰轮次合并为唯一的新摘要，再以流式 Chat Completions 调用模型并附带本地工具 Schema。文本 delta 立即进入临时展示状态；tool call delta 按 index 重组 ID、名称和参数，DeepSeek thinking 的 `reasoning_content` 单独重组但不进入可见文本。所有仍保留在历史中的 assistant 消息都会原样保存并回传该字段，因此工具结果续轮、完成门禁续轮和后续用户轮次都满足 thinking 协议。工具调用响应同时含有可见 `content` 时，Agent 在执行工具前发出有界 `decision_summary` 事件；WebRuntime 脱敏后将其原子替换流式草稿并持久化为折叠卡片。原始 `reasoning_content` 不进入网页状态；没有可见 `content` 时不生成摘要，也不会额外调用模型。上下文摘要请求仍使用非流式调用，不把摘要正文展示给用户。
...
8. 浏览器通过 `/api/events` 的 SSE 长连接接收带递增 revision 的状态快照，实时渲染模型文本、工具、审批和部署进度。流式回答保留既有消息 DOM，只增量更新最后一条 assistant 内容；侧栏数据未变化时也不重建节点。每五秒的 `/api/state` 轮询负责初始加载、断线恢复和旧浏览器兼容，较旧 revision 不会覆盖新状态。

## 对话与持久化

每个对话独立保存模型协议历史、网页展示条目、权限模式、工作目录关联、取消事件、运行状态、累计文件 Diff 和执行证据。网页展示条目可以是用户消息、决策摘要、工具结果、错误、系统通知或最终回答；决策摘要保存步骤号和有界工具名称，不保存工具参数。生成中的 assistant 文本仅存在于 `WebTask` 内存状态，最多约每 50 ms 唤醒一次 SSE 订阅者；决策摘要到达时会和流式草稿在同一次状态保存中切换，随后用户消息、压缩后的历史、每条完整工具结果和最终状态触发持久化检查点，避免按 token 高频写盘，同时缩短工具已修改磁盘但协议证据尚未保存的崩溃窗口。
