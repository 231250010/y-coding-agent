...
每个对话独立保存模型协议历史、网页展示条目、权限模式、工作目录关联、取消事件、运行状态和累计文件 Diff。生成中的 assistant 文本仅存在于 `WebTask` 内存状态，最多约每 50 ms 唤醒一次 SSE 订阅者；最终回答完成或任务失败时清空临时文本并一次性写入正式消息，避免高频磁盘写入和重复内容。
