...
def empty_session_state() -> dict[str, Any]:
    return {"version": 6, "current_id": None, "projects": [], "tasks": []}
...
def _normalize_task(task: dict[str, Any], project_id: str | None = None) -> dict[str, Any]:
    ...
    raw_execution_state = task_copy.get("execution_state")
    if isinstance(raw_execution_state, dict):
        task_copy["execution_state"] = deepcopy(raw_execution_state)
    else:
        task_copy.pop("execution_state", None)
    return task_copy
